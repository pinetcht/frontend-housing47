...
export default function Users() {
    ...
    useEffect(() => {
        ...
        const fetchUserData = async () => {
            try {
                ...
                // Get current user
                const response = await axios.get(`https:/on-render-server-name/users/get_user/${userId}`);
                // Get available users
                const availableResponse = await axios.get(`https:/on-render-server-name/users/available/${userId}`);
    ...
    const handleAddToGroup = async (roommateId) => {
        try {
            ...
            const response = await axios.post("https:/on-render-server-name/users/addToGroup", {
                currId: userId,
            ...
            if (response.status === 200) {
                ...
                const updatedResponse = await axios.get(`https:/on-render-server-name/users/get_user/${userId}`);
                setUserData(updatedResponse.data);

                // Update the available users list
                const updatedUsersResponse = await axios.get('https:/on-render-server-name/users');
    ...
    const handleLeaveGroup = async () => {
        try {
            ...
            const response = await axios.post(`https:/on-render-server-name/users/leaveGroup/${userId}`);
